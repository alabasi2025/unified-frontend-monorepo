...
@Component({
  ...
  template: `
    <div class="p-4">
      ...
      <p-button label="إضافة صنف" icon="pi pi-plus" (onClick)="showAddDialog()"></p-button>
      ...
      <p-dialog [(visible)]="displayDialog" [header]="editMode ? 'تعديل صنف' : 'إضافة صنف جديد'" [modal]="true" [style]="{width: '500px'}">
        <div class="p-fluid">
          <div class="field">
...
export class ItemsComponent implements OnInit {
  ...
  saveRole() {
    ...
    const operation = this.editMode 
      ? this.itemsService.update(this.currentRole.id, this.currentRole)
      : this.itemsService.create(this.currentRole);

    operation.subscribe({
      next: () => {
        this.messageService.add({ severity: 'success', summary: 'نجح', detail: this.editMode ? 'تم تحديث الصنف' : 'تم إضافة الصنف' });
      ...
      error: (error) => {
        ...
        this.saving = false;
        this.messageService.add({ severity: 'error', summary: 'خطأ', detail: 'فشل حفظ الصنف' });
      }
    });
  }
  ...
  deleteRole(role: any) {
    this.confirmationService.confirm({
      message: `هل أنت متأكد من حذف الصنف "${role.name}"؟`,
      header: 'تأكيد الحذف',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'نعم',
      rejectLabel: 'لا',
      accept: () => {
        this.itemsService.delete(role.id).subscribe({
          next: () => {
            this.messageService.add({ severity: 'success', summary: 'نجح', detail: 'تم حذف الصنف' });
            this.loadRoles();
          },
          error: (error) => {
            console.error('Error deleting role:', error);
            this.messageService.add({ severity: 'error', summary: 'خطأ', detail: 'فشل حذف الصنف' });
          }
        });
      }
    });
