...
@Component({
  ...
  template: `
    <div class="p-4">
      ...
      <p-button label="إضافة وحدة" icon="pi pi-plus" (onClick)="showAddDialog()"></p-button>
      
      <p-table [value]="units" [loading]="loading" styleClass="p-datatable-gridlines mt-3">
      ...
      <p-dialog [(visible)]="displayDialog" [header]="editMode ? 'تعديل وحدة' : 'إضافة وحدة جديد'" [modal]="true" [style]="{width: '500px'}">
        <div class="p-fluid">
          <div class="field">
...
export class UnitsComponent implements OnInit {
  ...
  saveRole() {
    ...
    const operation = this.editMode 
      ? this.unitsService.update(this.currentRole.id, this.currentRole)
      : this.unitsService.create(this.currentRole);

    operation.subscribe({
      next: () => {
        this.messageService.add({ severity: 'success', summary: 'نجح', detail: this.editMode ? 'تم تحديث الوحدة' : 'تم إضافة الوحدة' });
      ...
      error: (error) => {
        ...
        this.saving = false;
        this.messageService.add({ severity: 'error', summary: 'خطأ', detail: 'فشل حفظ الوحدة' });
      }
    });
  }
  ...
  deleteRole(role: any) {
    this.confirmationService.confirm({
      message: `هل أنت متأكد من حذف الوحدة "${role.name}"؟`,
      header: 'تأكيد الحذف',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'نعم',
      rejectLabel: 'لا',
      accept: () => {
        this.unitsService.delete(role.id).subscribe({
          next: () => {
            this.messageService.add({ severity: 'success', summary: 'نجح', detail: 'تم حذف الوحدة' });
            this.loadRoles();
          },
          error: (error) => {
